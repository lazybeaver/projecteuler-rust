...
const GRID_X: isize = 20;
const GRID_Y: isize = 20;
const GRID_INPUT: [isize; 400] = [
...
// The maximum consecutive neighbours to look at
const NEAREST_N: isize = 4;

// Directions in which traversal is allowed
...
fn largest_product() -> isize {
    let mut largest_p: isize = std::isize::MIN;
    for x in 0..GRID_X {
        for y in 0..GRID_Y {
            for d in DIRECTIONS.iter() {
                let mut current_p = 1;
                for n in 0..NEAREST_N {
                    let a: isize = x + (d.0 * n);
                    let b: isize = y + (d.1 * n);
                    if a > -1 && a < GRID_X && b > -1 && b < GRID_Y {
                        let index: usize = (a as usize * GRID_X as usize) + b as usize;
                        current_p = current_p * GRID_INPUT[index];
                    }
                }
                if current_p > largest_p {
                    largest_p = current_p;
                }
            }
        }
...
fn main() {
    let result = largest_product();
    println!("Largest Product: {:?}", result);
}

#[test]
fn test_largest_product() {
    assert_eq!(70600674, largest_product());
}
